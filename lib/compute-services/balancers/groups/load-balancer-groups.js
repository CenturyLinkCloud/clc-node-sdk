...
function SharedLoadBalancers(dataCenterService, loadBalancerClient, queueClient) {
    ...
    function modifyPools(modificationConfig) {
        if (modificationConfig.pool) {
            return Promise.all(
                _.map(_.asArray(modificationConfig.pool), _.partial(tryToUpdatePool, modificationConfig))
            );
        }
        return Promise.resolve();
    }

    function tryToUpdatePool(groupConfig, poolConfig) {
        if (poolConfig.id) {
            return self._poolService().modify({id: poolConfig.id}, _.omit(poolConfig, 'id'));
        } else {
            return self._poolService().find({balancer: _.omit(groupConfig, 'pool')})
                .then(function(pools) {

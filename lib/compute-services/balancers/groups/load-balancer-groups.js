...
function SharedLoadBalancers(dataCenterService, loadBalancerClient, queueClient) {
    ...
    self.create = function(command) {
        ...
        return new NoWaitOperationPromise(queueClient, processedBalancerRef, "Create Shared Load Balancer").from(result);
    };
    ...
    function modifySingle(modificationConfig, balancer) {
        if (_.isUndefined(modificationConfig.name)) {
            modificationConfig.name = balancer.name;
        }

        if (_.isUndefined(modificationConfig.description)) {
     ...
     *
     * @param {SharedLoadBalancerCriteria} balancerCriteria - criteria that specify set of balancers that will be modified
     *
    ...
    self.modify = function (balancerCriteria, modificationConfig) {
        ...
        if (modificationConfig.enabled === true) {
            modificationConfig.status = "enabled";
        } else if (modificationConfig.enabled === false) {
            modificationConfig.status = "disabled";
        }

        modificationConfig = _.omit(modificationConfig, "enabled");

...
function SharedLoadBalancerNodes(loadBalancerPools, loadBalancerClient, queueClient) {
    ...
    self.create = function(command) {
        var result = loadBalancerPools.findSingle(command.pool)
            ...
            .then(function(enhancedPool) {
                return modifyNodesForPool(enhancedPool.pool, _.asArray(enhancedPool.nodes, command.nodes), command.nodes);
            });
    ...
    */
        ...
        var result = findPools(criteria)
            .then(function(enhancedPools) {
                return Promise.settle(
                    _.map(enhancedPools, function(enhancedPool) {
     ...
     * Must be a value between 1 and 65535.
     *
     * @return {Promise<LoadBalancerNodeCriteria[]>} - promise that resolved by list of references to
     ...
     *
     * @return {Promise<Array<LoadBalancerNodeCriteria>>} - promise that resolved by list of references to
     *                                            successfully processed resources.
     *
     * @instance
     * @function find

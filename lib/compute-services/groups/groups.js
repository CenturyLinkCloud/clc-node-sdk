...
function Groups(dataCenterService, groupClient, queueClient, accountClient) {
    ...
    function createHierarchy(config, parentGroupId) {
        //on first iteration it will be dataCenter
        if (parentGroupId instanceof Object) {
            parentGroupId = _.applyMixin(DataCenterMetadata, parentGroupId).getGroupId();
        }
        if (!isServerConfig(config)) {
            return createGroup(config, parentGroupId)
                .then(function(groupMetadata) {
                    config.groupId = groupMetadata.id;
                })
                .then(function() {
                    return Promise.all(_.map(config.subItems, _.partial(createHierarchy, _, config.groupId)));
                })

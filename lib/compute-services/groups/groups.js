...
function Groups(dataCenterService, groupClient, queueClient, accountClient) {
    ...
    self.getMonitoringStats = function(groupCriteria, filter) {
        return self
            ...
            .then(function (groups) {
                return Promise.all(_.map(groups, _.partial(getGroupMonitoringStats, filter)));
            });
    };

    function getGroupMonitoringStats(filter, group) {
        var monitoringStatsConverter = new MonitoringStatsConverter();
        var request = monitoringStatsConverter.validateAndConvert(filter);

        return groupClient
            .getGroupMonitoringStats(group.id, request);
    }

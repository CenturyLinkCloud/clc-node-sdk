...
//                .then(self.findSingle)
    ...
    function sendPowerOperationRequest(criteria, operation) {
        return self.find(criteria)
            ...
            .then(splitOperations)
            .then(Promise.all)
            .then(extractServerIdsFromStatus);
    }

    function splitOperations(statusList) {
        return _.map(statusList, function(status) {
            if (status.isQueued) {
                var promise = new OperationPromise(queueClient);
    ...
    function extractServerIdsFromStatus(statusList) {
        return _.map(statusList, function(status) {
            return {id: status.server};
        });
    }

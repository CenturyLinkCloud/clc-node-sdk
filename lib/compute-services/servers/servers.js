...
function Servers(serverClient, serverConverter, queueClient, groupService) {
    ...
    self.create = function (command) {
        var promise = doCreateServer(command);
    ...
    function doCreateServer(command) {
    ...
    self.modify = function (searchCriteria, command) {
        ...
        return new OperationPromise(queueClient, function() {
            return _.map(command.serversData, _.partial(_.omit, _, 'disks'));
        }, "Modify Server").fromInspections(result);
    };

    function modify(id, request) {
        return serverClient.modifyServer(id, request);
    }
            ...
            .then(function(server) {
                return Promise.all(
                    _.map(fetchPublicIpList(server), function(ip){
                        return serverClient.getPublicIp(server.id, ip);
                    })
                );
            });
    };
     ...
     * @param {ServerCriteria} searchCriteria - criteria that specify a set of servers
     * @returns {Promise} the queued operation
     *
     * @memberof Servers
     * @instance

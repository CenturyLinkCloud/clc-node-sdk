...
function Servers(serverClient, serverConverter, queueClient, groupService) {
    ...
    function sendPowerOperationRequest(criteria, operation) {
        var result = self.find(criteria)
            .then(function(servers) {
                return serverClient.powerOperation(operation, _.pluck(servers, "id"));
            });
     ...
     */
    self.findPublicIp = function(searchCriteria) {
        var criteria = self._searchCriteriaFrom(arguments);
        var serverId;
        ...
        return Promise
            ...
            .then(function(server) {
                serverId = server.id;
                return _.filter(server.details.ipAddresses, function(ipAddress) {
                    return ipAddress.public;
                });
            })
            .then(function(ipAddresses) {
                return Promise.all(_.map(ipAddresses, function(ipAddress) {
                        return serverClient.getPublicIpAddress(serverId, ipAddress.public);
                    })
                );
            });
    };

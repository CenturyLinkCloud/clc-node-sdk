...
function Servers(serverClient, serverConverter, queueClient, groupService) {
    ...
    function loadServersById(criteria, servers) {
        ...
        if (criteria.id) {
            return Promise.join(
                Promise.all(
                    _.map(_.asArray(criteria.id), function(serverId) {
                        return self.findByRef({id: serverId});
                    })),
                servers
            );
        } else {
    ...
    function composeModifyServerPromise(command) {
        return Promise.resolve(command)
            ...
            .then(function(command) {
                var updateProperties = ['description', 'groupId', 'cpu', 'memory', 'disks', 'password'];
                ...
                if (command.disks) {
                    ...
                    var diskConfig = _.map(serverDisks, function(metadata) {
                        if (newDiskCfg.add) {
                            var toAdd = _.map(_.asArray(newDiskCfg.add), function(disk) {
                                disk.sizeGB = disk.size;
                                delete disk.size;
                                return disk;
                            });
                            metadata.disks = metadata.disks.concat(toAdd);
                        }

                        if (newDiskCfg.remove) {
                            var toRemove = _.asArray(newDiskCfg.remove);
                            metadata.disks = _.filter(metadata.disks, function(disk) {
                        ...
                        if (newDiskCfg.edit) {
                            ...
                            _.each(metadata.disks, function(srvDisk) {
                                var newDiskCfg = _.findWhere(toEdit, {id: srvDisk.id});
                                if (newDiskCfg) {
                                    srvDisk.sizeGB = newDiskCfg.size;
                                }
                            });
                        }

                        metadata.disks = _.map(metadata.disks, function(diskCfg) {
                            if (diskCfg.id) {
                                diskCfg.diskId = diskCfg.id;
                                delete diskCfg.id;
                            }
                            return diskCfg;
                        });
    ...
    self.modify = function (searchCriteria, command) {
        ...
        return self.find(searchCriteria)
            ...
            .then(function(serversData) {
                command.serverIds = _.map(serversData, _.property('id'));

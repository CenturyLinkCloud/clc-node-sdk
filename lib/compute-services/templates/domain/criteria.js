...
var DataCenterCriteria = require('./../../../common-services/datacenters/domain/criteria.js');
...
function TemplateCriteria (nakedCriteria) {
    ...
    function resolveFilterCriteria(criteria, condition) {
        var dataCenterPredicate = DataCenterCriteria.extractPredicateFromFilterCriteria(
            TemplateCriteria.extractDataCenterCriteria(criteria), "dataCenter"
        );
        var predicate = selectDefaultPredicate(condition);
        if (criteria.name) {
